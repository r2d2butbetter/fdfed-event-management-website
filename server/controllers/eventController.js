...
class eventController {
    ...
    // API route: Send filtered events as JSON for async frontend fetch
    async getEventsApi(req, res) {
    try {
        ...
        if (venueQuery) filter.venue = { $regex: venueQuery, $options: 'i' };
        ...
        console.log(`Returning ${events.length} events of total ${totalEvents}`);
    ...
    // Existing method to get all events and render with pagination (can be retained or replaced)
    async getAllEvents(req, res) {
        try {
            const titleQuery = req.query.title || '';
            const venueQuery = req.query.venue || '';
    ...
    // Get events by category and render
    async getEventsByCategory(req, res) {
        try {
            const category = req.params.category;
            const events = await Event.find({
    ...
    // Render form to create or edit event
    async createEventForm(req, res) {
        try {
            const { editId } = req.query;
            let event = null;
    ...
    // Get single event by ID and render page
    async getEventById(req, res) {
        try {
            const eventId = req.params.id;
            const event = await Event.findById(eventId).populate('organizerId');
